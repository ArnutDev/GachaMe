    ...
    setTimeout(async () => {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            if (chance <= 3) {
                ...
                if (result) {
                    rangersJson = await loadJSON('scraping/rate-up2/8-ultra-collab.json');
                    collab = true;
                } else {
                    rangersJson = await loadJSON('scraping/rate-up2/8-ultra.json');
                }
                grade = "Ultra 8 star";
            } else if (chance <= 8) {
                ...
                let result = checkValueInRange(value, range);
                if (result) {
                    rangersJson = await loadJSON('scraping/rate-up2/8-common-collab.json');
                    collab = true;
                } else {
                    rangersJson = await loadJSON('scraping/rate-up2/8-common.json');
                }
                grade = "8 star";
            } else {

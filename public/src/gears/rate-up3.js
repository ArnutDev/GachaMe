...
async function rateUp3() {
    ...
    setTimeout(async () => {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            let special = false;
            if (chance <= 1) {
                gearsJson = await loadJSON('json-data/gears/rate-up3/8c-info-special.json'); //change this
                ...
                if (result) {
                    gearsJson = await loadJSON('json-data/gears/rate-up3/8c-info-special.json');
                    special = true;
                } else {
                    gearsJson = await loadJSON('json-data/gears/rate-up3/8c-info.json');
                }
                grade = "8 star";
            } else if (chance <= 2) {
                gearsJson = await loadJSON('json-data/gears/rate-up3/7c-info-special.json'); // change this
                let amount = gearsJson.length;
                ...
                let result = checkValueInRange(value, range);
                if (result) {
                    gearsJson = await loadJSON('json-data/gears/rate-up3/7c-info-special.json');
                    special = true;
                } else {
                    gearsJson = await loadJSON('json-data/gears/rate-up3/7c-info.json');
                }
                grade = "7 star";
            } else if (chance <= 50) {
                gearsJson = await loadJSON('json-data/gears/rate-up3/6c-info.json');
                grade = "6 star";
            } else {

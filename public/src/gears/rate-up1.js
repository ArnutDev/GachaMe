...
async function rateUp1() {
    const divSlots = document.querySelectorAll('.content-display1'); // select 7 div
    ...
    setTimeout(async () => {
        ...
        for (let i = 0; i < 6; i++) {
            ...
            if (chance <= 1) {
                ...
                if (result) {
                    ...
                    gearsJson.push(specialJson[2]);
                    gearsJson.push(specialJson[3]);
                    // printItem(gearsJson)
                } else {
        ...
        // อัปเดตจำนวนรวม
        document.getElementById("normal-count1").innerHTML = ` ${count}, Ruby used: ${count/5 * 200}`;

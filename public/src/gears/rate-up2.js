    ...
    setTimeout(async () => {
        ...
        for (let i = 0; i < 6; i++) {
            ...
            if (chance <= 1) {
                ...
                if (result) {
                    //j=1 is 8 special have only 1 of them that rate up
                    gearsJson.push(specialJson[1]); //ใช้ตำแหน่งอาเรย์ในการบอกเรนเจอร์พิเศษ จะได้ใช้pathเดียวกันเลย ไม่ต้องก็อปวางหลายๆอัน
                    ...
                    //j= 0 and 2 is 2 of them that not rate-up now then move to none special
                    for (let j = 0; j < 3; j++) {
                        if (j == 1) { //skip none rate up
                            continue;
                        }
        ...
        // อัปเดตจำนวนรวม
        document.getElementById("normal-count2").innerHTML = ` ${count}, Free box: ${guaranteeCount}/${maxGuarantee}, Ruby used: ${count * 200}`;

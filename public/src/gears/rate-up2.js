...
async function rateUp2() {
    ...
    setTimeout(async () => {
        ...
        for (let i = 0; i < 7; i++) {
            ...
            let special = false;
            if (chance <= 1) {
                gearsJson = await loadJSON('json-data/gears/rate-up2/8c-info-special.json'); //change this
                ...
                gearsJson = await loadJSON('json-data/gears/rate-up2/7c-info-special.json'); // change this
                let amount = gearsJson.length;
                ...
                gearsJson = await loadJSON('json-data/gears/rate-up2/6c-info.json');
                grade = "6 star";
            } else {
